...
def analyze_donation_image(request):
    ...
    return JsonResponse({
        ...
        'safety': {
            ...
            'batch_visible': bool((ocr.get('batch_number') or '').strip()) and not safety.get('missing_batch'),
            'package_intact': safety.get('package_intact', not safety.get('damaged_packaging')),
            'seal_valid': safety.get('seal_valid', not safety.get('tampered_seal')),
            'label_readable': safety.get('label_readable', not safety.get('low_image_quality')),
